...
3D-DNA jobs should preferentially launch the actual `run-asm-pipeline.sh` payload through `srun --ntasks=1 --cpus-per-task=${SLURM_CPUS_PER_TASK:-24} --cpu-bind=none`, and wrap the payload with `ionice -c2 -n7 nice -n 10` to lower CPU/I/O scheduling priority on shared nodes. Do not pin the payload with `--cpu-bind=cores`, CPU masks/maps, `taskset`, or hard-coded CPU IDs by default: CPU performance on this host can vary substantially, and pinning can trap a long job on slow CPUs. Use affinity only for a user-requested controlled test.
