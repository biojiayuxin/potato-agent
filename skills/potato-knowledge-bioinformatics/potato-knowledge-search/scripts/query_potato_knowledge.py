#!/usr/bin/env python3
"""Query potato RAG and PlantScience.ai KG evidence.
...
def format_summary(data: dict[str, Any], args: argparse.Namespace) -> str:
    ...
    lines.extend([
        "",
        "Interpretation note: integrate the two sources only after labeling them separately. RAG snippets are potato literature retrieval evidence; PlantScience.ai KG entries are automatically extracted graph evidence.",
    ])
    return "\n".join(lines)
