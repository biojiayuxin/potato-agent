...
def register_provider(profile: ProviderProfile) -> None:
    ...
    if runtime_profile is not None and profile.name not in runtime_profile.model_providers:
        logger.warning(
            "Ignoring model provider %s: blocked by runtime profile %s",
            profile.name,
...
def _discover_providers() -> None:
    ...
    if _BUNDLED_PLUGINS_DIR.is_dir():
        for child in sorted(_BUNDLED_PLUGINS_DIR.iterdir()):
            ...
            if (
                ...
                and child.name not in runtime_profile.model_providers
            ):
                continue
    ...
    user_dir = None if runtime_profile is not None else _user_plugins_dir()
