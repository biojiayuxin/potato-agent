...
BUILTIN_TOOL_MODULES: Dict[str, tuple[str, ...]] = {
    "browser": (
        "tools.browser_tool",
    ...
    "discord_admin": ("tools.discord_tool",),
    ...
    "messaging": ("tools.send_message_tool",),
    "moa": ("tools.mixture_of_agents_tool",),
    ...
    "tts": ("tools.tts_tool",),
    "video": ("tools.vision_tools",),
    "video_gen": ("tools.video_generation_tool",),
    "vision": ("tools.vision_tools",),
    "web": ("tools.web_tools",),
    "search": ("tools.web_tools",),
    "x_search": ("tools.x_search_tool",),
    "yuanbao": ("tools.yuanbao_tools",),
}

_builtin_discovery_lock = threading.RLock()
_ensured_builtin_modules: Set[str] = set()


def _scan_builtin_module_names(tools_dir: Optional[Path] = None) -> List[str]:
...
def _import_builtin_modules(
    module_names: List[str], *, reattempt_cached: bool = False
) -> List[str]:
    imported: List[str] = []
    with _builtin_discovery_lock:
        for mod_name in module_names:
            if mod_name in _ensured_builtin_modules and not reattempt_cached:
                imported.append(mod_name)
                continue
            try:
                importlib.import_module(mod_name)
                _ensured_builtin_modules.add(mod_name)
                imported.append(mod_name)
...
def ensure_builtin_tools(
    ...
    *,
    ...
    """Incrementally import only modules needed by the requested toolsets.

    With no runtime profile, ``toolsets=None`` (or ``full=True``) preserves the
    upstream discover-all behavior.  An active runtime profile is a hard
    ceiling: callers cannot use this API to import disabled builtin modules.
    """
    profile = get_runtime_profile()
    if profile is not None:
        selected = profile.constrain_enabled_toolsets(toolsets)
        disabled = set(profile.merge_disabled_toolsets(disabled_toolsets))
        selected = [name for name in selected if name not in disabled]
        unknown = [name for name in profile.enabled_toolsets if name not in BUILTIN_TOOL_MODULES]
        if unknown:
            raise RuntimeProfileError(
                "runtime profile references toolsets without builtin module mappings: "
                + ", ".join(sorted(unknown))
            )
    elif full or toolsets is None:
        return _import_builtin_modules(_scan_builtin_module_names())
    else:
        disabled = set(disabled_toolsets or ())
        selected = [name for name in toolsets if name not in disabled]

    module_names = sorted(
        {
            module_name
            for toolset in selected
            for module_name in BUILTIN_TOOL_MODULES.get(toolset, ())
        }
    )
    return _import_builtin_modules(module_names)


def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    """Import built-in tools, preserving upstream discover-all compatibility."""
    if get_runtime_profile() is not None:
        return ensure_builtin_tools()
    return _import_builtin_modules(
        _scan_builtin_module_names(tools_dir),
        # A caller-supplied directory is a discovery/test extension point. Keep
        # the historic behavior of invoking import_module on every scan even
        # when another temporary directory used the same module name earlier.
        reattempt_cached=tools_dir is not None,
    )
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        if profile is not None:
            requested_names.intersection_update(profile.expected_tool_set)
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        """
        profile = get_runtime_profile()
        if profile is not None and name not in profile.expected_tool_set:
            return json.dumps({"error": f"Tool disabled by runtime profile: {name}"})
