...
def resolve_gateway_approval(session_key: str, choice: str,
    ...
    resolved at once (``/approve all``).  When *approval_id* is provided,
    only that exact request is resolved.  Legacy in-process callers that do
    not provide an ID continue to resolve the oldest request (FIFO).

    Returns the number of approvals resolved (0 means nothing was pending).
    Repeating an exact approval ID with the same choice returns 1 without
    touching the queue.  This lets an HTTP client safely retry after losing
    the first response.  Reusing the ID with a different choice returns 0.
    """
    normalized_approval_id = str(approval_id or "").strip()
    normalized_choice = str(choice or "").strip()
    with _lock:
        if normalized_approval_id:
            prior_choice = _gateway_resolved.get(session_key, {}).get(
                normalized_approval_id
            )
            if prior_choice is not None:
                return int(prior_choice == normalized_choice)
        ...
        if resolve_all:
            targets = list(queue)
            queue.clear()
        elif normalized_approval_id:
            target = next(
                (
                    entry
                    for entry in queue
                    if entry.approval_id == normalized_approval_id
                ),
                None,
            )
            if target is None:
