...
def _get_scratch_dir() -> Path | None:
    """Return a scratch directory only when the optional backend exists."""
    try:
...
def cleanup_all_environments():
    ...
    scratch_dir = _get_scratch_dir()
    import glob
    scratch_paths = [] if scratch_dir is None else glob.glob(str(scratch_dir / "hermes-*"))
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "never appear when piped. Available patterns include "
                            ...
                            "on `read_file` to read that file — "
