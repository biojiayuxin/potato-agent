...
def try_recover_primary_transport(
    ...
    try:
        # Validate the snapshot before closing the working client. A corrupt
        # or stale runtime must not leave the agent half-switched.
        rt = agent._primary_runtime
        from runtime_profile import validate_model_runtime

        validate_model_runtime(
            rt.get("provider"),
            rt.get("api_mode"),
            label="Primary recovery runtime",
        )
...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        validate_model_runtime(
            rt.get("provider"),
            rt.get("api_mode"),
            label="Primary restore runtime",
        )
        validate_model_runtime(
            rt.get("compressor_provider", rt.get("provider")),
            rt.get("compressor_api_mode"),
            label="Primary restore compressor runtime",
        )
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    from runtime_profile import get_runtime_profile, validate_model_runtime

    runtime_profile = get_runtime_profile()
    if runtime_profile is not None:
        new_provider = str(new_provider or "").strip().lower()
    validate_model_runtime(new_provider, label="Model switch target")
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    """
    from runtime_profile import runtime_profile_tool_error

    profile_error = runtime_profile_tool_error(function_name)
