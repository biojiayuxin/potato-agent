...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for tool_call in tool_calls:
        ...
        profile_block_result = runtime_profile_tool_error(function_name)

        # Reset nudge counters
        if profile_block_result is None and function_name == "memory":
            agent._turns_since_memory = 0
        elif profile_block_result is None and function_name == "skill_manage":
        ...
        if profile_block_result is None:
            try:
                from tools import tool_search as _ts
        ...
        if profile_block_result is not None:
            block_result = profile_block_result
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if profile_block_result is None and _block_msg is None:
            guardrail_decision = agent._tool_guardrails.before_call(function_name, function_args)
            if not guardrail_decision.allows_execution:
                _guardrail_block_decision = guardrail_decision

        _execution_blocked = (
            profile_block_result is not None
        ...
        if profile_block_result is not None:
            function_result = profile_block_result
            tool_duration = 0.0
