...
_SKILL_REVIEW_PROMPT = (
    "Review the conversation above for reusable knowledge worth saving "
    "in the skill library. Update skills when there is something new to "
    "capture or correct. If nothing is worth saving, say 'Nothing to save.' "
    "and stop.\n\n"
    ...
    "When saving knowledge, prefer the earliest action that fits:\n"
    ...
    "  4. CREATE A NEW CLASS-LEVEL UMBRELLA SKILL when the workflow "
    "is useful beyond the current task and no existing skill covers it, "
    "or when the user requests one. The name MUST be at the class level. "
    "The name MUST NOT be a specific PR number, error string, feature "
    "codename, library-alone name, or 'fix-X / debug-Y / audit-Z-today' "
    "session artifact. If the proposed name only makes sense for "
    "today's task, it's wrong — fall back to (1), (2), or (3).\n\n"
    "User-preference embedding: when saving a "
    "style/format/workflow preference to a skill, the update belongs in the "
    ...
    "Pinned skills CAN be improved — pin blocks deletion, not content "
    ...
    "If there is no new reusable knowledge or correction to save, "
    "say 'Nothing to save.' and stop."
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above for memory and skill updates worth saving.\n\n"
    "**Memory**: who the user is. Did the user reveal persona, "
    "desires, preferences, personal details, or expectations about "
    "how you should behave? Save facts about the user and durable "
    "preferences with the memory tool.\n\n"
    "**Skills**: how to do this class of task. Update skills when there "
    "is new reusable knowledge to capture or an existing instruction "
    "to correct.\n\n"
    "Target shape of the skill library: CLASS-LEVEL skills with a rich "
    "SKILL.md and a `references/` directory for session-specific detail. "
    "Not a long flat list of narrow one-session-one-skill entries.\n\n"
    "Signals to consider for a skill update:\n"
    ...
    "missing, or outdated.\n\n"
    "When saving knowledge to skills, prefer the earliest action that fits:\n"
    ...
    "  4. CREATE A NEW CLASS-LEVEL UMBRELLA when the workflow is "
    "useful beyond the current task and no existing skill covers it, "
    "or when the user requests one. "
    "Name at the class level — NOT a PR number, error string, "
    "codename, library-alone name, or 'fix-X / debug-Y' session "
    "artifact. If the name only fits today's task, fall back to (1), "
    "(2), or (3).\n\n"
    "User-preference embedding: when saving a workflow preference, "
    "put it in the relevant skill's SKILL.md body. Memory says 'who the user is and "
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Save new durable facts, reusable knowledge, or corrections where "
    "appropriate. If neither memory nor skills need an update, "
    "say 'Nothing to save.' and stop."
)
...
def _run_review_in_thread(
    ...
    review_messages: List[Dict] = []
    try:
        # Review runs in a thread; process-wide stream redirection is unsafe.
        ...
        # Teardown must not replace streams shared with foreground turns.
        if review_agent is not None:
            for cleanup in (review_agent.shutdown_memory_provider, review_agent.close):
                try:
                    cleanup()
                except Exception:
                    pass
