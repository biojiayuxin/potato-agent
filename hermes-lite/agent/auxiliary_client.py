...
def _enforce_runtime_auxiliary_policy(
    provider: Optional[str],
    api_mode: Optional[str] = None,
    *,
    main_runtime: Optional[Dict[str, Any]] = None,
) -> tuple[str, Optional[str]]:
    """Apply the process profile to every auxiliary-provider entry point."""
    from runtime_profile import RuntimeProfileError, get_runtime_profile

    profile = get_runtime_profile()
    if profile is None:
        return str(provider or ""), api_mode

    normalized = _normalize_aux_provider(provider)
    runtime = _normalize_main_runtime(main_runtime)
    if normalized in {"", "auto"}:
        main_provider = _normalize_aux_provider(
            runtime.get("provider") or _read_main_provider() or "custom"
        )
        normalized = "custom" if main_provider in {"", "auto"} else main_provider
    if normalized not in profile.model_providers:
        raise RuntimeProfileError(
            f"Auxiliary model provider {normalized!r} is not allowed by "
            f"runtime profile {profile.name!r}"
        )

    effective_mode = (
        str(api_mode or "").strip()
        or str(runtime.get("api_mode") or "").strip()
        or _RUNTIME_MAIN_API_MODE
        or "chat_completions"
    )
    if effective_mode not in profile.api_modes:
        raise RuntimeProfileError(
            f"Auxiliary API mode {effective_mode!r} is not allowed by runtime "
            f"profile {profile.name!r}"
        )
    return normalized, effective_mode
...
def resolve_provider_client(
    ...
    requested_auto = provider in {"", "auto"}
    provider, api_mode = _enforce_runtime_auxiliary_policy(
        provider, api_mode, main_runtime=main_runtime
    )
    from runtime_profile import get_runtime_profile

    runtime_profile = get_runtime_profile()
    if runtime_profile is not None and requested_auto:
        runtime = _normalize_main_runtime(main_runtime)
        explicit_base_url = (
            explicit_base_url
        ...
        explicit_api_key = (
            explicit_api_key
            or runtime.get("api_key")
            or _RUNTIME_MAIN_API_KEY
            or None
        )
        model = model or str(runtime.get("model") or "").strip() or _read_main_model()
    ...
    if provider == "custom":
        ...
        for try_fn in custom_resolvers:
...
def get_available_vision_backends() -> List[str]:
    ...
    if get_runtime_profile() is not None:
        return ["custom"] if _strict_vision_backend_available("custom") else []
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    requested, resolved_api_mode = _enforce_runtime_auxiliary_policy(
        requested, resolved_api_mode
    )
...
def _get_cached_client(
    ...
    if get_runtime_profile() is not None and _normalize_aux_provider(original_provider) in {"", "auto"}:
        profile_runtime = _normalize_main_runtime(main_runtime)
        base_url = (
            base_url
            or str(profile_runtime.get("base_url") or "").strip()
            or _RUNTIME_MAIN_BASE_URL
            or None
        )
        api_key = api_key or profile_runtime.get("api_key") or _RUNTIME_MAIN_API_KEY or None
        model = model or str(profile_runtime.get("model") or "").strip() or _read_main_model()
