...
def _merge_custom_provider_extra_body(agent, custom_providers: List[Dict[str, Any]]) -> None:
    ...
    from runtime_profile import validate_profile_request_overrides

    validate_profile_request_overrides(
    ...
    validate_profile_request_overrides(overrides)
...
def init_agent(
    ...
    if _runtime_profile is not None:
        enabled_toolsets = _runtime_profile.constrain_enabled_toolsets(enabled_toolsets)
        disabled_toolsets = _runtime_profile.merge_disabled_toolsets(disabled_toolsets)
        if provider and provider.strip().lower() not in _runtime_profile.model_providers:
            raise ValueError(
                f"Model provider {provider!r} is not allowed by runtime profile "
                f"{_runtime_profile.name!r}"
            )
        if api_mode and api_mode not in _runtime_profile.api_modes:
            raise ValueError(
                f"API mode {api_mode!r} is not allowed by runtime profile "
                f"{_runtime_profile.name!r}"
            )
        fallback_entries = (
            fallback_model
            if isinstance(fallback_model, list)
            else [fallback_model] if isinstance(fallback_model, dict) else []
        )
        for index, fallback in enumerate(fallback_entries):
            if not isinstance(fallback, dict):
                raise ValueError(
                    f"fallback_providers[{index}] must be a mapping under runtime "
                    f"profile {_runtime_profile.name!r}"
                )
            fallback_provider = str(fallback.get("provider") or "").strip().lower()
            if fallback_provider not in _runtime_profile.model_providers:
                raise ValueError(
                    f"Fallback model provider {fallback_provider!r} is not allowed "
                    f"by runtime profile {_runtime_profile.name!r}"
                )
            fallback_api_mode = str(fallback.get("api_mode") or "").strip()
            if (
                fallback_api_mode
                and fallback_api_mode not in _runtime_profile.api_modes
            ):
                raise ValueError(
                    f"Fallback API mode {fallback_api_mode!r} is not allowed by "
                    f"runtime profile {_runtime_profile.name!r}"
                )
    ...
    def _validate_profile_model_runtime() -> None:
        if _runtime_profile is None:
            return
        if agent.provider not in _runtime_profile.model_providers:
            raise ValueError(
                f"Effective model provider {agent.provider!r} is not allowed by "
                f"runtime profile {_runtime_profile.name!r}"
            )
        if agent.api_mode not in _runtime_profile.api_modes:
            raise ValueError(
                f"Effective API mode {agent.api_mode!r} is not allowed by "
                f"runtime profile {_runtime_profile.name!r}"
            )

    # Validate after URL/provider inference and before warming a transport so a
    # forbidden adapter is never imported as a side effect of auto-detection.
    _validate_profile_model_runtime()
    ...
    validate_profile_request_overrides(agent.request_overrides)
    ...
    try:
        ...
        _agent_cfg = {}
    if _runtime_profile is not None:
        _agent_cfg = dict(_agent_cfg or {})
        _agent_cfg["memory"] = {
            **dict(_agent_cfg.get("memory") or {}),
            "provider": "",
        }
        _agent_cfg["context"] = {
            **dict(_agent_cfg.get("context") or {}),
            "engine": _runtime_profile.context_engine,
        }
    try:
        agent._tool_guardrails = ToolCallGuardrailController(
    ...
    if _runtime_profile is not None and agent.tools is not None:
        # Provider-backed memory/context engines can inject schemas after the
        # registry selection pass. Reapply the name ceiling here so a future
        # compressor extension cannot silently expand the Potato surface.
        agent.tools = [
            tool
            for tool in agent.tools
            if tool.get("function", {}).get("name")
            in _runtime_profile.expected_tool_set
        ]
        agent.valid_tool_names.intersection_update(_runtime_profile.expected_tool_set)
        agent._context_engine_tool_names.intersection_update(
            _runtime_profile.expected_tool_set
        )
