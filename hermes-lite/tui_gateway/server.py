...
# ``session.create`` creates a new sid and never reattaches the old sid.
...
def _teardown_session(session: dict | None) -> None:
    ...
    Shared by ``session.close`` and the orphaned-session reaper.
    """
    if not session:
        return
...
def _load_enabled_toolsets() -> list[str]:
    from potato_hermes_lite.runtime_guard import require_potato_profile

    return list(require_potato_profile().enabled_toolsets)
...
def _restart_slash_worker(session: dict):
    """Classic slash workers are outside the Potato runtime boundary."""
    worker = session.get("slash_worker")
    if worker:
        try:
            worker.close()
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from potato_hermes_lite.skills import available_skills

        info["skills"] = available_skills()
    except Exception:
        pass
    try:
        from runtime_profile import get_runtime_profile
...
def _agent_cbs(sid: str) -> dict:
    return {
        ...
        "clarify_callback": lambda _question, _choices: "",
    }
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        return {
            ...
            "validated": False,
            "skipped": True,
            "message": "secret capture is not part of Potato Hermes Lite",
        }
...
def _(rid, params: dict) -> dict:
    ...
    if is_internal_session(db, target):
        if profile_home is not None:
            db.close()
        return _err(rid, 4007, "session not found")
...
def _(rid, params: dict) -> dict:
    del params
    return _err(rid, 5032, "Clipboard capture is not part of Potato Hermes Lite")
...
def _allowed_image_extensions() -> frozenset[str]:
    from potato_hermes_lite.attachments import IMAGE_EXTENSIONS
...
def _(rid, params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="pdf_attach_") as td:
        ...
        if raw_b64:
            ...
            from potato_hermes_lite.attachments import resolve_attachment_path

            resolved = resolve_attachment_path(raw_path)
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from potato_hermes_lite.attachments import detect_file_drop
...
def _(rid, params: dict) -> dict:
    del params
    return _err(rid, 5032, "Clarify interaction is not part of Potato Hermes Lite")


@method("sudo.respond")
def _(rid, params: dict) -> dict:
    del params
    return _err(rid, 5032, "Sudo interaction is not part of Potato Hermes Lite")


@method("secret.respond")
def _(rid, params: dict) -> dict:
    del params
    return _err(rid, 5032, "Secret interaction is not part of Potato Hermes Lite")
...
def _(rid, params: dict) -> dict:
    ...
    if not approval_id:
        return _err(rid, 4002, "approval_id required")
...
def _(rid, params: dict) -> dict:
    try:
        from potato_hermes_lite.provider_status import has_configured_runtime

        return _ok(rid, {"provider_configured": has_configured_runtime()})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        runtime = resolve_runtime_provider(requested=None)
        provider_configured = has_configured_runtime()
...
@method("reload.mcp")
def _(rid, params: dict) -> dict:
    return _err(rid, 5032, "MCP is not part of Potato Hermes Lite")
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    return _err(rid, 5032, "The classic Hermes CLI is not part of Potato Hermes Lite")
...
def _(rid, params: dict) -> dict:
    ...
    return _err(
        rid,
        4018,
        "slash.exec is not part of Potato Hermes Lite; use command.dispatch",
    )
...
def _voice_unavailable(rid) -> dict:
    return _err(rid, 5032, "Voice mode is not part of Potato Hermes Lite")


@method("voice.toggle")
def _(rid, params: dict) -> dict:
    del params
    return _voice_unavailable(rid)
...
def _(rid, params: dict) -> dict:
    ...
    return _err(
        rid,
        5032,
        "The Potato Hermes Lite tool surface is sealed by its runtime profile",
    )
...
def _(rid, params: dict) -> dict:
    del params
    return _err(rid, 5032, "Cron is not part of Potato Hermes Lite")
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "list":
            ...
            return _ok(rid, {"skills": available_skills()})
        if action == "search":
        ...
        if action == "install":
            return _err(
                rid,
                5032,
                "The classic Skills Hub UI is not part of Potato Hermes Lite; use skill_manage",
            )
        if action == "browse":
            return _err(
                rid,
