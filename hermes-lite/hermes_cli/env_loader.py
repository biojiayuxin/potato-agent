...
# Deployment policy passed in by a service manager is process authority, not
# user configuration. Preserve only keys that already existed before loading
# a user-owned .env; absent keys retain the normal upstream dotenv semantics.
_PROCESS_FIXED_ENV_KEYS = frozenset(
    {
...
def load_hermes_dotenv(
    *,
    ...
    fixed_environment = {
        key: os.environ[key]
        for key in _PROCESS_FIXED_ENV_KEYS
    ...
    # Secret-source discovery may consult runtime policy (for example before a
    # lazy installer runs). Restore service-manager policy before that happens
    # so the first process-wide profile lookup cannot cache user-owned values.
    os.environ.update(fixed_environment)
    _apply_external_secret_sources(home_path)
    # External secret sources must not be able to replace deployment policy.
    os.environ.update(fixed_environment)
