...
MAINTENANCE_BIND_HOST=${INTERFACE_BIND_HOST%%,*}
test -n "$MAINTENANCE_BIND_HOST"
sudoedit /etc/potato-maintenance.conf

test "$(stat -c '%U:%G:%a' /etc/potato-maintenance.conf)" = root:root:644
grep -Fx "bind_host = $MAINTENANCE_BIND_HOST" /etc/potato-maintenance.conf
...
`sudoedit` 时必须把 `bind_host` 设置为当前部署服务器上 Interface 的实际主监听 IPv4；HTTPS profile 通常是
`127.0.0.1`，显式 HTTP profile 则使用经 owner 批准的实际 LAN IPv4。该值属于站点配置，不得复制测试服务器
地址或写死到通用部署说明中。
...
维护页只保证部署服务器实际配置的监听地址和端口；外部反向代理或隧道能否继续访问取决于其上游配置。
已经加载且没有继续请求服务器的旧标签页不会被主动替换，用户刷新、导航或重新访问后才会看到维护页。
未发送草稿和进行中的上传不会由维护服务恢复。
