...
def test_file_stream_preserves_helper_maintenance_marker() -> None:
    command = [
        sys.executable,
        "-c",
        "import sys; sys.stderr.write('POTATO_USER_MAINTENANCE'); raise SystemExit(1)",
    ]
    with pytest.raises(interface_app_mod.PrivilegedMaintenanceError):
        interface_app_mod._open_file_stream(command)
...
def test_file_preview_returns_retryable_maintenance_error(monkeypatch) -> None:
    client, _ = _build_client_and_user(monkeypatch)
    monkeypatch.setattr(
        interface_app_mod,
        "_open_user_file_stream",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            interface_app_mod.PrivilegedMaintenanceError("retry later")
        ),
    )
    try:
        response = client.get("/api/files/preview/meta", params={"path": "notes.txt"})
...
def test_text_file_preview_returns_metadata_and_content(monkeypatch) -> None:
    ...
    try:
        meta_response = client.get(
            "/api/files/preview/meta", params={"path": "notes.md"}
        )
        ...
        text_response = client.get(
            "/api/files/preview/text", params={"path": "notes.md"}
        )
...
def test_log_style_files_preview_as_text(monkeypatch) -> None:
    ...
    try:
        for filename in filenames:
            meta_response = client.get(
                "/api/files/preview/meta", params={"path": filename}
            )
            assert meta_response.status_code == 200, meta_response.text
            assert meta_response.json()["preview_type"] == "text"

            text_response = client.get(
                "/api/files/preview/text", params={"path": filename}
            )
            assert text_response.status_code == 200, text_response.text
...
def test_bioinformatics_sequence_and_annotation_files_preview_as_text(
    monkeypatch,
) -> None:
    client, home_dir = _build_client_and_user(monkeypatch)
    _allow_direct_file_reads(monkeypatch)
...
def test_svg_preview_streams_inline_content(monkeypatch) -> None:
    ...
    try:
        meta_response = client.get(
            "/api/files/preview/meta", params={"path": "plot.svg"}
        )
        assert meta_response.status_code == 200, meta_response.text
        meta = meta_response.json()
        ...
        content_response = client.get(
            "/api/files/preview/content", params={"path": "plot.svg"}
        )
        ...
        assert (
            ...
            == 'inline; filename="plot.svg"'
        )
        assert content_response.headers["x-content-type-options"] == "nosniff"
        assert (
            "default-src 'none'" in content_response.headers["content-security-policy"]
        )
    finally:
        client.close()
...
def test_pdf_preview_streams_as_pdf(monkeypatch) -> None:
    ...
    try:
        meta_response = client.get(
            "/api/files/preview/meta", params={"path": "paper.pdf"}
        )
        assert meta_response.status_code == 200, meta_response.text
        assert meta_response.json()["preview_type"] == "pdf"

        content_response = client.get(
            "/api/files/preview/content", params={"path": "paper.pdf"}
        )
        assert content_response.status_code == 200, content_response.text
...
def test_preview_rejects_file_over_limit(monkeypatch) -> None:
    ...
    try:
        meta_response = client.get(
            "/api/files/preview/meta", params={"path": "large.txt"}
        )
        assert meta_response.status_code == 200, meta_response.text
        meta = meta_response.json()
        ...
        text_response = client.get(
            "/api/files/preview/text", params={"path": "large.txt"}
        )
...
def test_preview_rejects_invalid_relative_path(monkeypatch) -> None:
    ...
    try:
        response = client.get(
            "/api/files/preview/meta", params={"path": "../secret.txt"}
        )
